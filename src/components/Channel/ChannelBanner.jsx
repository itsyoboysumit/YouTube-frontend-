import React from 'react';
import { Camera } from 'lucide-react';
import { useAuth } from '../../hooks/useAuth';
import { updateAvatar, updateCoverImage } from '../../services/auth';

const ChannelBanner = () => {
  const { user, setUser } = useAuth();

  const avatarUrl = user?.avatar || 'https://via.placeholder.com/100';
  const coverImageUrl = user?.coverImage || null;

  const handleAvatarChange = async (e) => {
    const file = e.target.files[0];
    if (!file) return;

    try {
      const updatedUser = await updateAvatar(file);
      setUser(updatedUser); // update auth context with new avatar
    } catch (error) {
      console.error('Failed to update avatar:', error);
    }
  };

  const handleCoverImageChange = async (e) => {
    const file = e.target.files[0];
    if (!file) return;

    try {
      const updatedUser = await updateCoverImage(file);
      setUser(updatedUser); // update auth context with new cover image
    } catch (error) {
      console.error('Failed to update cover image:', error);
    }
  };

  return (
    <div
      className="relative h-55 bg-gray-300 dark:bg-gray-800"
      style={{
        backgroundImage: coverImageUrl ? `url(${coverImageUrl})` : undefined,
        backgroundSize: 'cover',
        backgroundPosition: 'center',
      }}
    >
      {/* Cover Image Update Icon */}
      <button
        onClick={() => document.getElementById('coverInput').click()}
        className="absolute top-2 right-2 bg-gray-100 dark:bg-gray-900 p-2 rounded-full border border-gray-300 hover:scale-105 transition"
        title="Change Cover Image"
      >
        <Camera size={18} />
      </button>
      <input
        id="coverInput"
        type="file"
        accept="image/*"
        className="hidden"
        onChange={handleCoverImageChange}
      />

      {/* Avatar */}
      <div className="absolute -bottom-12 left-6">
        <div className="relative">
          <img
            src={avatarUrl}
            alt="User Avatar"
            className="w-40 h-40 rounded-full border-4 border-white dark:border-black object-cover"
          />
          {/* Avatar Camera Icon */}
          <button
            onClick={() => document.getElementById('avatarInput').click()}
            className="absolute bottom-5 right-1 bg-gray-100 dark:bg-gray-900 p-1 rounded-full border border-gray-300 hover:scale-105 transition"
            title="Change Avatar"
          >
            <Camera size={16} />
          </button>
          <input
            id="avatarInput"
            type="file"
            accept="image/*"
            className="hidden"
            onChange={handleAvatarChange}
          />
        </div>
      </div>
    </div>
  );
};

export default ChannelBanner;
